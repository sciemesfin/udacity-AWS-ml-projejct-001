...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open(dogfile, "r") as infile:
        ...
        while line != "":
            # Process the line by stripping the newline character from the variable line
            line = line.strip()

            # Add the dogname (line) to dognames_dic if it doesn't already exist in the dognames_dic dictionary
            if line not in dognames_dic:
                dognames_dic[line] = 1

            # Reads the next line in the file to be processed with the while loop
    ...
    for key in results_dic:
        # Pet Image Label IS a Dog (e.g. found in dognames_dic)
        if results_dic[key][0] in dognames_dic:
            # Classifier Label IS an image of a Dog (e.g. found in dognames_dic)
